...
class BaseAuthenticatedConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        """Authenticate user via JWT from URL path and connect to the WebSocket group."""
    ...
    async def authenticate_user(self):
        """Extract token from URL path and authenticate user."""
        if not self.token:
            print("WebSocket authentication failed: No token provided in URL.")
            return None

        print(f"Token from URL: {self.token}")

        try:
            payload = decode(self.token, settings.SECRET_KEY, algorithms=["HS256"])
            user = await self.get_user(payload["user_id"])
...
class HarvestNotificationConsumer(BaseAuthenticatedConsumer):
    ...
    async def connect(self):
        ...
        await self.send(text_data=json.dumps(dummy_message))
...
class DeviceDataConsumer(AsyncWebsocketConsumer):
    ...
    def authenticate(self):
        try:
            if not self.token:
                raise ValueError("No token provided in URL")
            
            auth = JWTAuthentication()
            validated_token = auth.get_validated_token(self.token)
            self.user = auth.get_user(validated_token)
        except Exception as e:
            self.user = AnonymousUser()
            raise
